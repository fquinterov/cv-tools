...
-i, --invert_colors - create a calibration pattern with the colors inverted. Featured added by Fernando Quintero. 2024.
-H, --help - show help
"""
...
class PatternMaker:
    def __init__(self, cols, rows, output, units, square_size, radius_rate, page_width, page_height, markers, aruco_marker_size, dict_file, invert_colors):
        self.cols = cols
        self.rows = rows
        self.output = output
        self.units = units
    ...
    def make_circles_pattern(self):
        ...
        bg = SVG("rect", x=0, y=0, width=self.width, height=self.height, fill=self.bg_color, stroke="none")
        self.g.append(bg)

        for x in range(0, self.cols):
            for y in range(0, self.rows):
                dot = SVG("circle", cx=(x * spacing) + x_spacing + r,
                          cy=(y * spacing) + y_spacing + r, r=r, fill=self.main_color, stroke="none")
                self.g.append(dot)
            ...
            for y in range(0, self.rows):
                if x % 2 == y % 2:
                    square = SVG("rect", x=x * spacing + xspacing, y=y * spacing + yspacing, width=spacing,
                                 height=spacing, fill=self.main_color, stroke="none")
    ...
    def make_radon_checkerboard_pattern(self):
                ...
                if x % 2 == y % 2:
                    ...
                    if is_inside:
                        ...
                        square = SVG("path", d=self._make_round_rect(x * spacing + xspacing, y * spacing + yspacing,
                                      spacing, corner_types), fill=self.main_color, stroke="none")
                    self.g.append(square)
        if self.markers is not None:
            ...
            for x, y in self.markers: #fix color self.main_color
                color = "black"
                if x % 2 == y % 2:
                    color = "white"
                dot = SVG("circle", cx=(x * spacing) + x_spacing + r,
...
def main():
    ...
    parser.add_argument("-i", "--invert_colors", help="invert colors of the pattern", action="store", dest="invert_colors")
    ...
    pm = PatternMaker(columns, rows, output, units, square_size, radius_rate, page_width, page_height, markers, aruco_marker_size, dict_file, invert_colors)
